...
const PRESETS = [
  { id:'default',  name:'Roxo Neon',   sub:'Padrão',
    accent:'#bf00ff', accent2:'#7c3aed',
    dark: {bg:'#0a0010',surface:'#100020',surface2:'#1a003a',text:'#f8f5ff',muted:'#7a6a8a'},
    light:{bg:'#f5f0ff',surface:'#ffffff',surface2:'#ede5ff',text:'#1a0d2e',muted:'#7a6a8a'} },
  { id:'midnight', name:'Midnight',    sub:'Azul profundo',
    accent:'#6366f1', accent2:'#22d3ee',
    dark: {bg:'#040814',surface:'#0c1220',surface2:'#131d30',text:'#e0eaff',muted:'#5a7080'},
    light:{bg:'#eef2ff',surface:'#ffffff',surface2:'#e0e7ff',text:'#1e1b4b',muted:'#6070a0'} },
  { id:'ember',    name:'Ember',       sub:'Fogo',
    accent:'#f97316', accent2:'#ef4444',
    dark: {bg:'#0f0802',surface:'#1a1008',surface2:'#241808',text:'#fff1e0',muted:'#8a6040'},
    light:{bg:'#fff7ed',surface:'#ffffff',surface2:'#ffedd5',text:'#431407',muted:'#8a5030'} },
  { id:'matrix',   name:'Matrix',      sub:'Terminal',
    accent:'#00ff41', accent2:'#00cc33',
    dark: {bg:'#000800',surface:'#001200',surface2:'#002000',text:'#ccffcc',muted:'#2a6a2a'},
    light:{bg:'#f0fff0',surface:'#ffffff',surface2:'#e0ffe0',text:'#002000',muted:'#2a6a2a'} },
  { id:'gold',     name:'Ouro Negro',  sub:'Luxo',
    accent:'#ffd700', accent2:'#ff8c00',
    dark: {bg:'#080600',surface:'#120e00',surface2:'#1a1400',text:'#fff8e0',muted:'#806040'},
    light:{bg:'#fffbeb',surface:'#ffffff',surface2:'#fef3c7',text:'#1c1000',muted:'#806040'} },
  { id:'mono',     name:'Mono',        sub:'Minimalista',
    accent:'#e0e0e0', accent2:'#a0a0a0',
    dark: {bg:'#080808',surface:'#111111',surface2:'#1a1a1a',text:'#f0f0f0',muted:'#666666'},
    light:{bg:'#f5f5f5',surface:'#ffffff',surface2:'#ebebeb',text:'#111111',muted:'#888888'} },
];

let curTheme = { presetId:'default', mode:'dark', custom:{} };

/* ── Panel open / close ──────────────────── */
function openTheme()  { document.getElementById('overlay').classList.add('open'); }
function closeTheme() { document.getElementById('overlay').classList.remove('open'); }
function handleOverlay(e) { if (e.target === document.getElementById('overlay')) closeTheme(); }
...
function applyPreset(id) {
  curTheme.presetId = id; curTheme.custom = {};
  applyTheme(); buildPresetGrid(); saveTheme();
}
...
function applyTheme() {
  ...
  const vars = {
    '--neon': acc, '--p300': acc, '--p400': acc,
    '--p200': lighten(acc, 0.3),
    '--neon2': ac2, '--p500': darken(acc, 0.2), '--p600': darken(acc, 0.4),
    '--p900': bg, '--p800': b.surface, '--p700': b.surface2,
    '--white': txt, '--muted': b.muted,
  };
...
function applyCustomColor(key, val) {
  curTheme.custom[key] = val; curTheme.presetId = 'custom';
...
function saveTheme() { try { localStorage.setItem('vltx_theme', JSON.stringify(curTheme)); } catch (_) {} }

function loadTheme() {
  try {
    const s = localStorage.getItem('vltx_theme');
    if (s) { const x = JSON.parse(s); curTheme = { ...curTheme, ...x }; }
